namespace Hydrospanner.Phases.Transformation
{
	...
	public sealed class TransformationHandler : IEventHandler<TransformationItem>
	{
		...
		{
			var live = false;
			var local = this.item.ForeignId == Guid.Empty;
			...
			this.buffer.AddRange(this.deliveryHandler.Deliver(this.item.Body, this.item.Headers, this.item.MessageSequence, live, local));
			...
			for (var i = 0; i < this.buffer.Count; i++)
			{
				...
				this.buffer.AddRange(this.deliveryHandler.Deliver(this.buffer[i], BlankHeaders, this.item.MessageSequence + 1 + i, live, true));
			}
