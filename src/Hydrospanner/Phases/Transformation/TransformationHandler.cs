namespace Hydrospanner.Phases.Transformation
{
	...
	public sealed class TransformationHandler : IEventHandler<TransformationItem>
	{
		...
		private bool Transform()
		{
			...
			var messages = this.deliveryHandler1.Deliver(this.item.Body, this.item.Headers, this.item.MessageSequence, live).ToList();
			this.buffer.AddRange(messages);

			// deliveryHandler1 ensures that messages are gathered from the hydratables only once we reach the live stream
			for (var i = 0; i < this.buffer.Count; i++)
			{
				...
				var messages2 = this.deliveryHandler1.Deliver(this.buffer[i], BlankHeaders, this.item.MessageSequence + 1 + i, live).ToList();
				this.buffer.AddRange(messages2);
			}
		...
		public TransformationHandler(
			...
			IDeliveryHandler deliveryHandler1,
			ISystemSnapshotTracker snapshot)
		{
			...
			if (deliveryHandler1 == null)
				throw new ArgumentNullException("deliveryHandler1");
			...
			this.deliveryHandler1 = deliveryHandler1;
		...
		private readonly IDeliveryHandler deliveryHandler1;
