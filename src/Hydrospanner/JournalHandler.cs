namespace Hydrospanner
{
	...
	public sealed class JournalHandler : IEventHandler<DispatchMessage>
	{
		public void OnNext(DispatchMessage data, long sequence, bool endOfBatch)
		{
			...
			if ((endOfBatch && this.buffer.Count > 0) || this.buffer.Count >= 524)
		...
		private void JournalMessages()
		{
			...
			using (var command = connection.CreateCommand())
			{
				...
				for (var i = 0; i < this.buffer.Count; i++)
				{
					...
					command.WithParameter("@wire{0}".FormatWith(i), item.WireId.ToNull(), DbType.Guid); // to null
					command.WithParameter("@type{0}".FormatWith(i), item.SerializedType, DbType.String);
					command.WithParameter("@payload{0}".FormatWith(i), item.SerializedBody, DbType.Binary);
					command.WithParameter("@headers{0}".FormatWith(i), item.SerializedHeaders, DbType.Binary); // can be null
		...
		private const string AppendMessage = "INSERT INTO messages (wire_id, type_name, payload, headers) SELECT @wire{0}, @type{0}, @payload{0}, @headers{0};\n";
